      ...
      users: [
        {
          ...
          password:
            "$2b$10$hA0ay8TXM/8C5HrwBA.KJ.dRDeiVb7pe91nm5RbLZpCzlOoeQGdN.", // 1234@Kunj
          firstName: "System",
...
app.get("/api/data", authenticateToken, (req, res) => {
  ...
  switch (userRole) {
    ...
    case "teacher":
      ...
      const teacherFaculty = database.faculty.find(
        (f) => f.email === req.user.email
      );

      if (teacherFaculty) {
        const teacherSubjects = database.subjects.filter(
          (s) => s.assignedFaculty === teacherFaculty.name
        );
        const teacherTimetables = database.savedTimetables.filter((t) => {
          return Object.values(t.timetable || {}).some((day) =>
            Object.values(day || {}).some(
              (slot) => slot && slot.faculty === teacherFaculty.name
            )
          );
        });
    ...
    case "student":
      ...
      if (studentData.department && studentData.semester) {
        studentTimetables = database.savedTimetables.filter(
          (t) =>
            t.department === studentData.department &&
            t.semester === studentData.semester
        );
      }
...
app.put(
  ...
  (req, res) => {
    ...
    if (
      database.departments.includes(trimmedName) &&
...
app.delete(
  ...
  (req, res) => {
    ...
    const usedInFaculty = database.faculty.some(
      (f) => f.department === departmentName
    );
    const usedInSubjects = database.subjects.some(
    ...
    if (usedInFaculty || usedInSubjects) {
      return res.status(400).json({
        error:
          "Cannot delete department. It is being used by existing faculty or subjects.",
      });
    }
...
app.delete(
  ...
  (req, res) => {
    try {
      ...
      const index = database.savedTimetables.findIndex(
        (t) => t.id === timetableId
      );
      if (index !== -1) {
...
app.post(
  ...
  (req, res) => {
    ...
    if (writeDatabase(database)) {
      ...
      res
        .status(500)
        .json({ error: "Failed to add course-department combination" });
    }
  }
);
...
app.delete(
  ...
  (req, res) => {
    ...
    const index = database.courseDepartments.findIndex(
      (cd) => cd.id === courseDepartmentId
    );

    if (index === -1) {
      return res
        .status(404)
        .json({ error: "Course-Department combination not found" });
    }
    ...
    database.courses = [
      ...new Set(database.courseDepartments.map((cd) => cd.course)),
    ];
    database.departments = [
      ...new Set(database.courseDepartments.map((cd) => cd.department)),
    ];

    if (writeDatabase(database)) {
      res.json(deleted);
      console.log(
        "🗑️ Course-Department deleted:",
        `${deleted.course} - ${deleted.department}`
      );
    } else {
      res
        .status(500)
        .json({ error: "Failed to delete course-department combination" });
    }
  }
);
...
app.listen(PORT, () => {
  ...
  console.log(
    `💡 Visit http://localhost:${PORT} to access your timetable generator`
  );
});
